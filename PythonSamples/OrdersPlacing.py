...
class OrderPlacingSample(CELEnvironment.CELSinkBase):
    ...
    def Start(self):
        ...
        Trace("Calculating a price that under the market by 10 tick sizes...")
        buyOrderPrice = bestBid.Price - 10 * dispatchedInstrument.TickSize
        Trace("It is {}".format(buyOrderPrice))

        Trace("Create buy limit order")
        buyOrder = win32com.client.Dispatch(self.celEnvironment.cqgCEL.CreateOrder(constants.otLimit, self.instrument,
                                                                                   self.account, 1,
