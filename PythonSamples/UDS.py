# The sample demonstrates how to defind UDS and place a strategy order
...
class UDSPlacingSample(CELEnvironment.CELSinkBase):
    def __init__(self):
        self.account = None
    ...
    def Start(self):
        ...
        Trace("UDS waiting...")
        AssertMessage(self.eventUDSIsReady.wait(TIMEOUT), "UDS resolution timeout!")

        dispatchedUDS = win32com.client.Dispatch(self.uds)
        dispatchedOrder = win32com.client.Dispatch(
            self.celEnvironment.cqgCEL.CreateStrategyOrder(constants.otLimit, dispatchedUDS, self.account, None, 1,
...
CELEnvironment.StartSample(UDSPlacingSample)
