...
# We need to use the datetime lib and other 2 functions below to calculate the
...
def request_bar_time():
    client_msg = ClientMsg()
    request = client_msg.time_bar_request.add()
    request.request_id = 1
    request.time_bar_parameters.contract_id = 1
    request.time_bar_parameters.bar_unit = minutely_index
    ...
    request.time_bar_parameters.from_utc_time = current_milli_time()\
                - base_time_milli_time() - bars_number * milliseconds_in_minute
    client.send_client_message(client_msg)
    while True:
        server_msg = client.receive_server_message()
        if server_msg.time_bar_report[0].is_report_complete is True:
            break
